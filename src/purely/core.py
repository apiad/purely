...
def ensure[T](
    value: Optional[T], error: Union[str, Exception] = ValueError("Value is None")
) -> T:
    """
...
class Chain[T]:
    ...
    def ensure(
        self, error: Union[str, Exception] = ValueError("Chain value is None")
    ) -> "Chain[T]":
...
class Option[T]:
    ...
    def unwrap(
        self, error: Union[str, Exception] = ValueError("Called unwrap on None")
    ) -> T:
